...
public class SolverFactory {
    ...
    public static SolutionCode solve(String algo, AlgoPara para) {
        return selectAlgo(algo, para).solve();
    }

    private static IDS selectAlgo(String algo, AlgoPara para) {
        if (algo.equalsIgnoreCase("cfpsst")) {
            return new CFPSST(para.getMaxGen(), DataLab.driverList(path + "vehicle.xls"),
                    DataLab.taskList(path + "task.xls"), para.getSize(), para.getFitnessType(), para.getAlpha(), para.getBeta(), para.getGama(), para.getN(), para.getB1(), para.getB2());
        } else if (algo.equalsIgnoreCase("cfps")) {
            return new CFPS(para.getMaxGen(), DataLab.driverList(path + "vehicle.xls"),
                    DataLab.taskList(path + "task.xls"), para.getSize(), para.getFitnessType(), para.getAlpha(), para.getBeta(), para.getN());
        } else if (algo.equalsIgnoreCase("ga")) {
            return new GaIDS(para.getMaxGen(), DataLab.driverList(path + "vehicle.xls"),
                    DataLab.taskList(path + "task.xls"), para.getSize(), para.getFitnessType(), para.getN());
        } else if (algo.equalsIgnoreCase("pso")) {
            return new PsoIDS(para.getMaxGen(), DataLab.driverList(path + "vehicle.xls"),
                    DataLab.taskList(path + "task.xls"), para.getSize(), para.getFitnessType());
        } else if (algo.equalsIgnoreCase("abc")) {
            return new ABCIDS(para.getMaxGen(), DataLab.driverList(path + "vehicle.xls"),
                    DataLab.taskList(path + "task.xls"), para.getFitnessType());
        }
        return new SsIDS(para.getMaxGen(), DataLab.driverList(path + "vehicle.xls"),
                DataLab.taskList(path + "task.xls"), para.getSize(), para.getFitnessType(), para.getB1(), para.getB2(), para.getN(), true);
    }
}
